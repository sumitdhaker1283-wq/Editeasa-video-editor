...
[UPLOADS, THUMBS, SEGMENTS, OUTPUTS, PUBLIC].forEach(d=>{ if(!fs.existsSync(d)) fs.mkdirSync(d); });
...
// Thumbnails generator (uses ffmpeg screenshots)
app.post('/thumbnails', async (req,res) => {
  try {
    ...
    ffmpeg.ffprobe(infile, (err, metadata) => {
      ...
      const n = Math.max(4, Math.min(120, parseInt(count) || 20));
      const step = dur / n;
      const promises = [];
      for(let i=0;i<n;i++){
...
// Serve static assets (public, uploads, thumbs, segments, outputs)
...
// Split video into segments at given timestamps
...
// Concatenate segments (server-side)
